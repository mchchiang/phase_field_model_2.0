...
int main (int argc, char* argv[]) {
  ...
  if (argc != 10) {
    cout << "Usage: angle_corr npoints nc coeff startTime "
  ...
  int nc = stoi(string(argv[++argi]), nullptr, 10);
...
#pragma omp parallel default(none)					\
  shared(nbins, npoints, nc, angle, cosAvg, count, endShiftBin) private(ibin)
  {
#ifdef _OPENMP
...
#pragma omp for schedule(dynamic,10)
	...
	ibin = j-i;
	for (int n = 0; n < npoints; n++) {	  
	  cosAvg[id][ibin] += cos(nc*(angle[j][n]-angle[i][n]));
	}
